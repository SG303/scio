...
async def update_deck(
    deck_id: int, deck_data: FlashcardDeckUpdate, db: AsyncSession = Depends(get_db)
):
...
async def update_card(
    card_id: int, card_data: FlashcardUpdate, db: AsyncSession = Depends(get_db)
):
...
async def get_study_cards(
    ...
    session_id: Optional[int] = Query(
        None, description="Optional session ID for resume"
    ),
    ...
    return StudyQueueResponse(
        ...
        cards=[
            FlashcardStudyResponse(id=c.id, front=c.front, back=c.back, state=c.state)
            for c in cards
        ],
    )


@router.get("/decks/{deck_id}/incomplete-session")
async def get_incomplete_session_info(deck_id: int, db: AsyncSession = Depends(get_db)):
    """
    Check if there's an incomplete session for this deck.
    Returns session info if found, null if not.
    """
    session = await get_incomplete_session(db, deck_id)
    ...
    if session.cards_studied_json:
        try:
            cards_studied = json.loads(session.cards_studied_json)
    ...
    return {
        ...
        "cards_studied_count": len(cards_studied),
...
async def submit_review(
    card_id: int,
    review: ReviewSubmit,
    session_id: Optional[int] = Query(None, description="Session ID to track progress"),
    db: AsyncSession = Depends(get_db),
):
